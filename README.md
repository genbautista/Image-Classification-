...
The model is evaluated by precision and recall, it produces a confusion matrix displaying the correclty predicted images over the overall false positives, and precision score is displayed as a percentage to the right.
Recall is calculated by finding the overall true positives (correctly predicted) over the total number of that classes images in the dataset.

Accuracy is measured by dividing the correctly matched images by the total test images and is displayed for each parallelization method.
...
## Prerequisites
To run this project you must have rustc version at least 1.73.0, we reccomend using 1.93.1 or the latest version available.
...
We reccomend having at least 8 CPU cores to run this project, as there are 8 image classes and for parallelization you want at least 8 threads that won't have to wait on each other
